...
echo ""
grep -F -f $cwd/$outfolder/genelist.corrected $annotation | awk '$3 == "mRNA" {print}' > $cwd/$outfolder/genes_entries.gff
...
grep -F -f $cwd/$outfolder/genelist.corrected $annotation | awk '$3 == "mRNA" {print}' | cut -f1 | grep -vFf - $cwd/$outfolder/genelist.corrected > $cwd/$outfolder/unmatched_genes
...
seqtk seq -L 100 $cwd/$outfolder/promoters_800nt.fa.tmp > $cwd/$outfolder/promoters_800nt.fa.tmp.tmp
...
python3 /projects/aliemelo/resources/scripts/fimo_tsv_to_gff.py --tsv $cwd/$outfolder/fimo.out/fimo.qvalue0.01.tsv
...
python3 /projects/aliemelo/resources/scripts/merged_bed_to_csv.py --bed $cwd/$outfolder/fimo.out/fimo.qvalue0.01.merged_overlaps.gff
python3 /projects/aliemelo/resources/scripts/fimo_prep.py --file $cwd/$outfolder/fimo.out/fimo.qvalue0.01.merged_overlaps.csv --out $cwd/$outfolder/fimo.out/mapping.results --alt T
